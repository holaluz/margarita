...
👀 The use of MaText for list items is **recommended**. By default, the color and size are set according to the properties specified in the MaTextList component specified above (tone/size). If a custom configuration is needed, tone and size for the designated MaText can be set as explained [here](https://holaluz.github.io/margarita/?path=/story/components-text--text).
